    ...
    switch (shape.type) {
        ...
        case 'frame':
            ...
            if (shape.type === 'rect' || shape.type === 'frame') {
                ...
                ctx.beginPath()
                ctx.ellipse(relativeX + shape.w / 2, relativeY + shape.h / 2, shape.w / 2, shape.h / 2, 0, 0, Math.PI * 2)
                ctx.stroke()
            }
...
export const exportGeneratedUIAsPNG = async (
    ...
    try {
        ...
        if (contentDiv) {
            ...
            throw new Error('No content div to export !!')
        }
        canvas.toBlob(
            (blob) => {
                if (blob) {
        ...
        console.log('❌ Failed to create GeneratedUI snapshot', error)
